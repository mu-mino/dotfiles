...
export async function complete(textDocument, position, config) {
    ...
    if (token && (isBasicExpression(token) || isPotentiallyExpression(token, isAction))) {
        ...
        const context = isAction
            ...
            : await getWorkflowExpressionContext(namedContexts, config?.contextProviderConfig, workflowContext, Mode.Completion);
    ...
    let values = await getValues(token, keyToken, parent, config?.valueProviderConfig, workflowContext, indentString, schema);
    ...
    if (!config?.featureFlags?.isEnabled("allowCopilotRequestsPermission") &&
 ...
 */
async function getValues(token, keyToken, parent, valueProviderConfig, workflowContext, indentation, schema) {
    if (!parent) {
        return [];
    }
    ...
    const values = definitionValues(def, indentation, keyToken ? DefinitionValueMode.Key : DefinitionValueMode.Parent, tokenStructure, schema);
    return filterAndSortCompletionOptions(values, existingValues);
}
/**
 ...
 */
function getEscapeHatchCompletions(token, keyToken, indentation, position, schema) {
...
function getExpressionCompletionItems(token, context, extensionFunctions, pos, featureFlags) {
    ...
    try {
        return completeExpression(expressionInput, context, extensionFunctions, validatorFunctions, featureFlags).map(item => mapExpressionCompletionItem(item, currentInput[cursorOffset]));
    }
    catch (e) {
