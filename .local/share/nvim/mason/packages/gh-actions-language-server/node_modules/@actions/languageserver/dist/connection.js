...
export function initConnection(connection) {
    ...
    connection.onCompletion(async ({ position, textDocument }) => {
        return timeOperation("completion", async () => await onCompletion(connection, position, getDocument(documents, textDocument), client, repos.find(repo => textDocument.uri.startsWith(repo.workspaceUri)), cache, featureFlags));
    });
