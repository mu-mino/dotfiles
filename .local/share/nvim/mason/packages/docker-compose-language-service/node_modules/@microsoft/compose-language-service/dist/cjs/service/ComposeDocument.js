...
class ComposeDocument {
    yamlDocument = new Lazy_1.Lazy(() => this.buildYamlDocument());
    documentSettings;
    ...
    async getSettings() {
        ...
        if (!this.documentSettings) {
            ...
            if (ctx.clientCapabilities?.experimental?.documentSettings?.request) {
                const result = await ctx.connection.sendRequest(DocumentSettingsClientCapabilities_1.DocumentSettingsRequest.type, { textDocument: this.id });
                if (result) {
    ...
    guessDocumentSettings() {
        ...
        const eol = /\r/ig.test(documentText) ? DocumentSettingsClientCapabilities_1.CRLF : DocumentSettingsClientCapabilities_1.LF;
